...
def in_keyring(recipient):
    ...
    valid_keys=list(filter(lambda k:k.revoked==0 and k.expired==0, matched_keys))
    if(len(valid_keys)==0):
        return False
    else:
        return True
...
def get_selected_key(keys):
    ...
    all_keys=list(keys)
    valid_keys=list(filter(lambda k:k.revoked==False and k.expired==False, all_keys))
            ...
            with open(seal_path, "w") as outfile:
                try:
                    ...
                    if(answer=='y' or answer=='yes'):
                        _cyphertext, _result, _sign_result  = c.encrypt(infile,
                        recipients=seal_list, sign=True,
